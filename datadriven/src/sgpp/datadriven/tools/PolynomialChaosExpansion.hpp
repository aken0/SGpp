    ...
    class PolynomialChaosExpansion{
      ...
      const std::map<std::string, std::function<double(double)>> denoms{
        {"hermite",[](double j){return std::sqrt(2*M_PI);}},
          {"jacobi",[this](double j)->double{return 0;}},
          {"legendre",[](double j){return 2/((2*j)+1);}},
          {"laguerre",[](double j){return std::exp(-j);}},
          {"genlaguerre",[this](double j) ->double {return std::pow(j,alpha)*std::exp(-j);}}
      };
      ...
      public:
      PolynomialChaosExpansion(std::function<double(const base::DataVector&)> func,int order, std::vector<std::string> types, double alpha,double beta);
      ...
      double monteCarloQuad(std::function<double(double)> func, double left, double right, long N);
